import { getProjectBySlug } from '../modules/api.js';
...
function section(icon, title, body) {
  return `
    <section class="detail-section">
      <h2 class="detail-section__title"><span class="detail-section__icon">${icon}</span>${escapeHtml(title)}</h2>
      ${body}
    </section>
  `;
}
    ...
    <div class="container">
      <div class="detail-grid">
        <div>
          ...
          ${section('🛠️', 'Approach & how it was built', `<p>${escapeHtml(p.approach)}</p>`)}
          ${section('🧱', 'Development challenges', list(p.developmentChallenges, 'warn'))}
          ${section('⚠️', 'Mistakes made', list(p.mistakes, 'danger'))}
    ...
    root.innerHTML = `
      <div class="container" style="padding-block:var(--space-16)">
        <div class="empty-state">
          ...
          <h1 class="empty-state__title">No project selected</h1>
          <p class="empty-state__text">Pick a project from the archive to view its full documentation.</p>
      ...
      root.innerHTML = `
        <div class="container" style="padding-block:var(--space-16)">
          <div class="empty-state">
            ...
            <p class="empty-state__text">We couldn't find a project with the slug "<code>${escapeHtml(slug)}</code>".</p>
